...
[(DSD)קוד חילוץ מבנה הנתונים](#(DSD)-קוד-חילוץ-מבנה-הנתונים)
[פלט מהקוד: תרשים מבנה בסיס הנתונים](#פלט-הקוד:-תרשים-מבנה-בסיס-הנתונים)
[תרשים-ישויות-ויחסים(ERD)](#תרשים-ישויות-ויחסים(ERD))
## שלב א
### אודות-הפרויקט
...
### אינטגרציה-ומבטים
מטרה:
בשלב זה ביצענו אינטגרציה עם מאגר מידע נוסף שמתאר פעילות ציוד ואנשים במתחם חדרי הכושר.
...
### (DSD)קוד-חילוץ-מבנה-הנתונים
לצורך תיעוד טכני של שלב זה, כתבנו סקריפט פייתון שעובר על הגיבוי ומייצר ממנו את מבנה טבלת בסיס הנתונים באופן טקסטואלי

קובץ הקוד המלא:
extract_dsd.py

הקוד עובר על הגדרות מתוך שאילתות הגיבוי ומפיק רשימה של שמות טבלאות, שמות עמודות וסוגי הנתונים שלהן. מהם יצרנו את התרשימים(dsd and erd)


### פלט-הקוד:-תרשים-מבנה-בסיס-הנתונים
<img src="DBProject_0939_2584/שלב_ג/extracted_DSD.png" width="700"/>
התרשים נוצר בצורה אוטומטית על ידי הסקריפט והוא מאפשר להבין בצורה גרפית את המבנה הקיים במערכת.

### תרשים-ישויות-ויחסים(ERD)
<img src="DBProject_0939_2584/שלב_ג/corrected_ERD.png" width="700"/>
נבנה על בסיס התרשים הקודם ומבנה העמודות. הוא מתעד קשרים היררכיים, אילוצים בין טבלאות (כגון ירושה והרכבה), וממחיש את ההיגיון העסקי של המערכת.
